...
A zero-recurring-cost Twitch stream monitor: detects when watched channels go live, builds a real-time chaptered timeline of each stream, and ranks clip-worthy moments from chat activity and stream audio. A hard project constraint is **no metered/usage-based billing** — only the free Twitch Helix API, anonymous IRC chat, local audio decoding (streamlink + ffmpeg), and local heuristic analysis (regex + statistics + DSP). Do not introduce paid APIs (LLMs, transcription services, etc.) without explicit approval.
